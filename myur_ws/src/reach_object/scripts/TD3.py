...
class TD3(object):
    ...
    def Learn(self):
        ...
        if self.gpu:
            a_ = self.actor_target(s_) + torch.clamp(torch.from_numpy(self.noise_target.sample()).float(),
                                                            min=-self.noise_clip,max=self.noise_clip).cuda()
        else:
            a_ = self.actor_target(s_) + torch.clamp(torch.from_numpy(self.noise_target.sample()).float(),
    ...
    def save_model(self,model_dir,model_name, epoch):
        torch.save(self.actor.state_dict(), (model_dir+model_name+'actor_%d.pth' %epoch))
        torch.save(self.critic.state_dict(), (model_dir+model_name+'critic_%d.pth' %epoch))
        torch.save(self.optim_a.state_dict(), (model_dir+model_name+'optim_a_%d.pth' %epoch))
        torch.save(self.optim_c.state_dict(), (model_dir+model_name+'optim_c_%d.pth' %epoch))


    def load_model(self,model_dir,model_name, index):
        self.actor_target.load_state_dict(torch.load(model_dir+model_name+'actor_%d.pth' %index))
        self.critic_target.load_state_dict(torch.load(model_dir+model_name+'critic_%d.pth' %index))
        self.actor.load_state_dict(torch.load(model_dir+model_name+'actor_%d.pth' %index))
        self.critic.load_state_dict(torch.load(model_dir+model_name+'critic_%d.pth' %index))
        self.optim_a.load_state_dict(torch.load(model_dir+model_name+'optim_a_%d.pth' %index))
        self.optim_c.load_state_dict(torch.load(model_dir+model_name+'optim_c_%d.pth' %index))
